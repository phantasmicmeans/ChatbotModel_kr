...
class Conversation():
    ...
    def pad(self, seq, max_len, start=None, eos=None):
        if start:
            padded_seq = [config.GO_ID] + seq
        elif eos:
            padded_seq = seq + [config.EOS_ID]
        else:
            padded_seq = seq
...
#token=twitter.morphs(line.strip())
                tokens=self.Tokenizer(line.strip()) # twitter로 token 생성시 이 라인을 주석처리하고, 위 라인의 주석을 풀어야합니다.
